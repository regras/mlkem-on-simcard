...
public class MLKEMApplet extends Applet {
     ...
     */
    private static void absorb(byte[] inBuf, short inOff, short inLen, short rate, byte padding) {
        if (isSqueezing) {
            reset();
        }
     ...
     * @param padding the padding rule (either SHAKEpadding or SHA3padding)
     */
    private static void squeeze(byte[] outBuf, short outOff, short outLen, short rate, byte padding) {
        if (!isSqueezing) {
     ...
     */
    private static byte[] getRoundConstant(short r) {
        return switch (r) {
            case 0 -> rc0;
            case 1 -> rc1;
            case 2 -> rc2;
     ...
     */
    private static void and64(byte[] a, short aOffset, byte[] b, short bOffset, byte[] result, short offset) {
     ...
     * @param offset offset into the result array
     */
    private static void xor64(byte[] a, short aOffset, byte[] b, short bOffset, byte[] result, short offset) {
        result[offset] = (byte) (a[aOffset] ^ b[bOffset]);
        result[(short) (offset + 1)] = (byte) (a[(short) (aOffset + 1)] ^ b[(short) (bOffset + 1)]);
        result[(short) (offset + 2)] = (byte) (a[(short) (aOffset + 2)] ^ b[(short) (bOffset + 2)]);
        result[(short) (offset + 3)] = (byte) (a[(short) (aOffset + 3)] ^ b[(short) (bOffset + 3)]);
     ...
     * @param offset offset into array a
     */
    private static void flip64(byte[] a, short offset) {
     ...
     */
    private static void littleEndian64(byte[] input, short inOffset, byte[] output, short outOffset, byte buff) {
        // Swap pos 0 and 7
     ...
     */
    private static void baseMultiplier(short a0, short a1, short b0, short b1, short zeta, short[] array, short roffset) {
     ...
     */
    private static void compressPoly(short[] polyA, short offset, int paramsK, byte[] result, short roffset, byte[] t) {
    ...
    private static void decompressPoly(byte[] a, short aoffset, int paramsK, short[] result, short offset, byte[] t) {
        short aa = aoffset;
        switch (paramsK) {
            case 2:
            case 3:
     ...
     * @param rOffset the starting offset in the output buffer
     */
    private static void polyToBytes(short[] a, short offset, byte[] result, short rOffset) {
        short t0, t1;
     ...
     */
    private static void polyFromBytes(byte[] a, int aOffset, short[] result, short offset) {
        for (short i = 0; i < paramsN / 2; i++) {
     ...
     */
    private static void polyFromData(byte[] msg, short[] result, short rOffset) {
        short mask;
        for (short i = 0; i < paramsN / 8; i++) {
            for (short j = 0; j < 8; j++) {
     ...
     */
    private static void polyToMsg(short[] a, short offset, byte[] msg, short msgOffset) {
        int LOWER = paramsQ >>> 2;
     ...
     * @param rOffset the starting offset in the output polynomial array
     */
    private static void getNoisePoly(byte[] seed, short seedOffset, byte nonce, int paramsK, short[] result, short rOffset) {
        int l;
        if (paramsK == 2) {
     ...
     */
    private static void polyBaseMulMont(short[] polyA, int aOffset, short[] polyB, int bOffset, short[] result, short roffset) {
     ...
     * @param offset the starting offset in the polynomial array
     */
    private static void polyToMont(short[] poly, short offset) {
     ...
     */
    private static void polyReduce(short[] poly) {
     ...
     * @param offset the starting offset in the polynomial array
     */
    private static void polyReduce(short[] poly, short offset) {
        for (short i = 0; i < paramsN; i++) {
     ...
     * @param offset the starting offset in the polynomial array
     */
    private static void polyConditionalSubQ(short[] poly, short offset) {
        for (short i = offset; i < paramsN + offset; i++) {
     ...
     */
    private static void polyAdd(short[] polyA, short[] polyB) {
     ...
     * @param bOffset the starting offset in polyB
     */
    private static void polyAdd(short[] polyA, short aOffset, short[] polyB, short bOffset) {
        for (short i = 0; i < paramsN; i++) {
     ...
     * @param polyB the second polynomial array
     */
    private static void polySub(short[] polyA, short[] polyB) {
     ...
     * @param bOffset the starting offset in polyB
     */
    private static void polySub(short[] polyA, short aOffset, short[] polyB, short bOffset) {
        for (short i = 0; i < paramsN; i++) {
            polyA[(short) (aOffset + i)] = (short) (polyA[(short) (aOffset + i)] - polyB[(short) (bOffset + i)]);
        }
    }
     ...
     */
    private static void compressPolyVectorElement(short[] polySingle, int paramsK, byte[] outC, short outOffset, short[] temp) {
     ...
     */
    private static void decompressPolyVectorElement(byte[] inC, short inOffset, int paramsK, short[] outPoly, short outOffset) {
        if (paramsK == 2 || paramsK == 3) {
     ...
     */
    private static int convertByteTo32BitUnsignedInt(byte[] x, int off) {
     ...
     */
    private static int convertByteTo24BitUnsignedInt(byte[] x, int off) {
        int r = (x[(short) off] & 0xFF);
        r = r | ((x[(short) ++off] & 0xFF) << 8);
        r = r | ((x[(short) ++off] & 0xFF) << 16);
        return r;
     ...
     * @param roffset the starting offset in the output array
     */
    private static void generateCBDPoly(byte[] buf, int paramsK, short[] result, short roffset) {
     ...
     */
    private static short conditionalSubQ(short a) {
        a = (short) (a - paramsQ);
     ...
     */
    private static short generateUniform(short[] uniformR, short roffset, byte[] buf, short bufOffset, int bufl, int l) {
        int d1;
        int d2;
     ...
     */
    private static void generatePRFByteArray(int l, byte[] seed, short seedOffset, byte nonce, byte[] bufHash) {
        reset();
     ...
     * DEVELOPER NOTE: For this function, on-demand polynomial generation was used to reduce the memory footprint
     * @param paramsK the security parameter (2 for ML-KEM-512, 3 for ML-KEM-768, 4 for ML-KEM-1024)
     ...
     */
    private static void encrypt(byte[] m, byte[] coins, int paramsK, byte[] C) {
     ...
     * @param paramsK the security parameter (2 for ML-KEM-512, 3 for ML-KEM-768, 4 for ML-KEM-1024)
     */
    private static void decrypt(byte[] message, int paramsK) {
        for (short i = 0; i < paramsN; i++) {
            bufPolyTemp[i] = 0;
        }
