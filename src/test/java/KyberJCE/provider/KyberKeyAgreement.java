...
import KyberJCE.provider.kyber.KyberParams;
import KyberJCE.spec.KyberParameterSpec;
import KyberJCE.util.KyberKeyUtil;
...
public final class KyberKeyAgreement extends KeyAgreementSpi {
    ...
    public void engineInit(Key key, AlgorithmParameterSpec params,
        ...
        KyberPrivateKey kyberPrivKey;
        kyberPrivKey = (KyberPrivateKey) key;
    ...
    public Key engineDoPhase(Key key, boolean lastPhase)
        ...
        if (key instanceof KyberPublicKey) {
            ...
            kyberPubKey = (KyberPublicKey) key;
...
//                return this.decrypt768(cipherText);
     ...
     */
    public KyberDecrypted decrypt768(byte[] ciphertext, byte[] privateKey) throws NoSuchAlgorithmException {
        int paramsK = 3;
        byte[] sharedSecretFixedLength = new byte[KyberParams.KyberSSBytes];
     ...
     * @return The decrypted data
     */
    public KyberDecrypted decrypt1024(byte[] ciphertext, byte[] privateKey) throws IllegalArgumentException, NoSuchAlgorithmException {
        int paramsK = 4;
        byte[] sharedSecretFixedLength = new byte[KyberParams.KyberSSBytes];
     ...
     * @return KyberEncrypted
     */
    public KyberEncrypted encrypt768(byte[] variant, byte[] publicKey) throws NoSuchAlgorithmException, InvalidKeyException {
        variant = verifyVariant(variant);
        KyberEncrypted msg = new KyberEncrypted();
        int paramsK = 3;
