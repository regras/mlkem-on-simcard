package KyberJCE.provider.kyber;
...
import KyberJCE.provider.KyberPackedPKI;
import KyberJCE.provider.KyberUniformRandom;
...
public final class Indcpa {
    ...
    public static KyberPackedPKI generateKyberKeysAlt(int paramsK, byte[] publicSeed) {
        KyberPackedPKI packedPKI = new KyberPackedPKI();
        try {
            short[][] skpv = Poly.generateNewPolyVector(paramsK);
            short[][] pkpv = Poly.generateNewPolyVector(paramsK);
            ...
            MessageDigest h = MessageDigest.getInstance("SHA3-512");
            byte[] seedInput = Arrays.copyOf(publicSeed, publicSeed.length + 1);
            seedInput[publicSeed.length] = (byte) paramsK; // paramsK is 2/3/4 in this codebase
            byte[] fullSeed = h.digest(publicSeed);
